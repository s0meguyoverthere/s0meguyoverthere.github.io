...
function JSONcanvas(){
    let canvas = document.getElementById("ppdisplaywindow");
    let ctx = canvas.getContext("2d");
    let img = new Image();   // Create new img element
    img.src = "DB/00001/pa.jpg"; // Set source path
...
function JSONcine () {
    ...
    // Clear Node to prepare for new data
        ...
        for (let i=0; i <= jmax; i++) {
            jimagepath = "DB/00001/cine/" + i + ".jpg";
            jcine[i] = new Image();
            jcine[i].src = jimagepath;
        }

        //Create Node to display image
        const imgnode = document.createElement('img'); // define new image
        imgnode.setAttribute("src", "DB/00001/cine/0.jpg");
        imgnode.setAttribute("id", "ppdisplaycontent");
        imgnode.setAttribute("height", "100%");
        imgnode.setAttribute("width", "100%");
        imgnode.style.transform = "translate(-50%.-50%)";
        imgnode.style.position = "absolute";
...
function pointerdownevent (ev){
        ijy = ev.offsetY;   // pointer y location on click
...
function trackmousemovement (ev) {
    ...
    if (scrollstate) {
        ...
        jy = ev.offsetY;
        djy = ijy - jy;
        djyd = Math.floor(djy/jthresh);
        j = ijc + djyd;
        if (j>jmax) {j=jmax};
        if (j<0) {j=0};

        document.getElementById("sometexthere").innerHTML = 'j = ' + j + ' and ijy = ' + ijy + ' and ijc = ' + ijc + ' and jy = ' + jy + ' and djy = ' + djy + ' and djyd = ' + djyd;
        document.getElementById("ppdisplaycontent").src = jcine[j].src;
        // if (Math.abs(djy) < jthresh) {
        //     if (djy < 0) {revcine();}
        //     if (djy > 0) {forcine();}
        // }
        // ijy = jy;
...
function checkkey(x){
  ...
  if(keycode == 38)  forcine(); // 38 key = up arrow
  if(keycode == 40)  revcine(); // 40 key = down arrow
  if(keycode == 73)  imginvert(); // i key invert image
}

function imginvert() {
    document.getElementById("ppdisplaycontent").style.filter="invert(100%)";
}
...
function forcine(){ // advance forward in cine
    ...
    if (j < jmax) {
        ...
        // keep j=jmax index for shown image
    } 
}
...
function revcine(){ // go back in cine
    ...
    if (j > 0) {
        ...
        // do nothning, keep first image
        //document.getElementById("img"+ (j-1)).style.visibility = "visible"; // show next image (reduce display artifact)
        //document.getElementById("img"+j).style.visibility = "hidden"; // hide current image
        // keep j = 0 index for shown image
    }
}